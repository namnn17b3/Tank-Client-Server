...
public class Client extends JPanel implements ActionListener {
    ...
    public void paint(Graphics g) {
        ...
        if (player2Bullet != null && player2Shoot) {
            ...
            && (br.checkCollision(player2Bullet.getX(), player2Bullet.getY())
        ...
        dataSend = new Data(player1X, player1Y, player1right, player1left, player1down, player1up,
                player1score, player1lives, player1Shoot, bulletShootDir1, player1Bullet, br, rpl1, pause, soundSign1);
        ...
        try {
            ...
            g.drawString("Server disconnected!", 670, 300);
        }
        try {
            ois = new ObjectInputStream(socket.getInputStream());
            dataReceive = (Data) ois.readObject();
            player2X = dataReceive.getPlayerX();
            player2Y = dataReceive.getPlayerY();
            player2right = dataReceive.getPlayerright();
            player2left = dataReceive.getPlayerleft();
            player2down = dataReceive.getPlayerdown();
            player2up = dataReceive.getPlayerup();
            player2score = dataReceive.getPlayerscore();
            player2lives = dataReceive.getPlayerlives();
            player2Shoot = dataReceive.getPlayerShoot();
            bulletShootDir2 = dataReceive.getBulletShootDir();
            player2Bullet = dataReceive.getPlayerBullet();
            brickOnTmp = dataReceive.getBrick().getBickOn();
            for (int i = 0; i < 42; i++) {
                brickON[i] = brickON[i] & brickOnTmp[i];
            }
            rpl2 = dataReceive.getRpl();
            pause = Math.max(dataReceive.getPause(), pause);
            soundSign2 = dataReceive.getSoundSign();
        ...
        if (hasException == false) {
            g.drawString("Connected to Server!", 670, 300);
        }
